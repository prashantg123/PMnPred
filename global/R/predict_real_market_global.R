...
Predict <- function(HFdata, current, time.index) {
  ...
  if (!is.data.frame(matched)) {
    ...
    patterns.filtered <- patterns.sub[abs(patterns.sub$t12 - (pt2-pt1)) < 30 & abs(patterns.sub$dy21 - (points[pt2] - points[pt1])) < 0.01
                                      & abs(patterns.sub$vol12 - volatility12) < 0.01, ]  # Put more filtering
    ...
    # Generate new time index after current point
    future.time <- seq(time.index[current], 
                       time.index[current] + as.difftime((ceiling(pdt3)+24)*interval, units = "mins"), 
                       by=paste(interval,"mins"))
    new.time <- c(time.index[1:(current-1)], future.time)
  
    first <- pt1-24
    first <- ifelse(first<1, 1, first)
    last <- pt3+24
    
    df <- data.frame(x = first:current, y = points[first:current])
    ...
    p <- ggplot(df, aes(x = x, y = y)) + geom_line() + 
      ...
      scale_y_continuous(breaks = round(seq(llim, ulim, by = 0.02), 2), 
                         limits = c(llim, ulim)) +
      scale_x_continuous(breaks = seq(1, length(new.time), by = 24), 
                         limits = c(first, last), 
                         labels = as.character(new.time[seq(1, length(new.time), by = 24)]))
